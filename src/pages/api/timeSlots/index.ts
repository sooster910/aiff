  ...
  try {
    ...
    if (Object.keys(req.query).length === 0) {
      const timeslots = JSON.parse(
        fs.readFileSync("db/timeSlots.json", "utf-8")
      );
    ...
    response = filteredByStoreId.filter(
      (timeslot) => Number(timeslot.regularClassId) === Number(regularClassId)
    );
  } catch (err) {
    console.log("err", err);
  }
