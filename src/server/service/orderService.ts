  ...
  async processOrderApproval(order: OrderDTO) {
    ...
    try {
      ...
      if (paymentResult.status !== "DONE") {
        throw new Error(
          `토스 결제 승인 상태가 올바르지 않음,${paymentResult.status}`
        );
      }
      ...
      if (!isDev) {
        // sms, slack전송
        await this.context.slackNotificationService.notifySlack({ text });
        await this.context.kakaoNotificationService.notifyKakao({
          regularClassName,
          startDate,
          qty,
          storeName,
      ...
      if (err instanceof Error) {
        const errorText = `⛑️ /orderService/processOrderApproval error: ${err.message}-${customerName}:\n${orderId}\npaymentKey: ${paymentKey}\n 폰번호:${phone}\n가격:${amount}* ${qty}`;
        if (!isDev) {
          await this.context.slackNotificationService.notifySlack({
            text: errorText,
          });
        } else {
